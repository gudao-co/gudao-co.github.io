...
import { withdraw, getBalance as getSkillBalance } from 'gudao-co-core/dist/skill';

function SkillWithdraw() {
  ...
  const updateWalletBalance = (curr?: Currency) => {
    ...
    if (curr && wallet) {
      ...
      Promise.all([getSkillBalance(id, curr), getBalance(wallet, wallet.addr, curr)]).then((rs) => {
        setBlanances(rs)
  ...
  const onSubmit = () => {
    ...
    setErrmsg('')
    setTX(undefined)
    withdraw(id!, newValue(amount, currCurrency!), (s) => {
      if (s.name === 'tx') {
        setTX(s.tx!)
      } else if (s.title) {
        setProgress(s.title)
